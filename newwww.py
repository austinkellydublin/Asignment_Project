...
class Route:
    ...
    def __init__(self, port1, port2, port3, port4, port5, craft='777'):
        ...
        self.portlist = [self.port1.code, self.port2.code, self.port3.code, self.port4.code, self.port5.code,
                         self.port6.code]  # a convenience array of airports from submitted route
        self.craft = Aircraft(craft)  ##create an aircraft object from the craft parameter
        self.candidates = Permroutes(self.port1.code, self.port2.code, self.port3.code, self.port4.code,
                                     self.port5.code).candidates  # permroutes generates the alternative paths and stores in a dictionary
        ...
        self.cleanup_candidate_routes()  # certain alternative paths have legs beyond fuel tank capacity or two stops are the same port so these are removed
        self.portsinrange()  # for each alternative route we need a list of ports in range for each stop
    ...
    def cleanup_candidate_routes(
            self):  ####GET RID OF ROUTES THAT HAVE A LEG GREATER THAN FUEL TANK CAPACITIY or HAVE SAME PORT LISTED TWICE IN SUCCESSION
    ...
    def portsinrange(
            self):  # FOR EACH ROUTE GENERATE A SET OF SUBROUTES WITH AIRPORT AHEAD WITHIN FLYING RANGE OF ANY GIVEN PORT FOR EACH HUB
        ...
        for routenumber, route in self.candidates.items():  ## process each route left remaining after clean up in the candidate routes dictionary
            ...
            if routenumber < 24:  # for five legged journeys generate the legs and populate the candidates_legs dictionary
                ...
                aportssubroute = []  ##each port will have its own list of ports in reach

                for port in range(
                        5):  # process all ports ahead of the stopped at port to see if reachable with full fuel tank

                    fuel = maxfuelrange  # from each stopped at port can fuel up -so want to see how many ports and their codes can reach on full tank
                    # print('start fuel',fuel)
                    aportssubroute = []
                    for leg in range(port,
                                     5):  # inner loop  will retake off from just landed port and see if can reach next station
                        if legsdist[
                            leg] > fuel:  # if distance to next port beyond reach append the subroute as is and break
    ...
    def cheapest(self):  # this will generate and store the cheapest route form all the different permutations
        self.cheapest_distance = 0000000000000000
        self.costofcheapest = 999999.9999999
        self.cheapestlegs = []
        self.cheapest_cost = 999999999999
        self.cheapest_route = []
        self.fuelstrategy = []
        ...
        for routenumber, route in self.candidates.items():
            ...
            if routenumber >= 24:  ##for routes with six legs
                ...
                for stopped_at_port in range(6):  # for each leg of the journey
                    ...
                    if stopped_at_port == 0:  # if at the very first port
                        ...
                        fuelused = legs[
                            stopped_at_port - 1]  ##fuel used up is fuel travelling from  from last port to current port
                    ...
                    for idx2, aheadports in enumerate(
                            subroute):  ## see if there is a cheaper port ahead within range cycle through ports looking for a cheaper one
                        ...
                        if (airports[
                                aheadportsindex].currencyeurorate <= cheapestrate):  ##find first cheapest airport as will buy enough fuel to get to it
                            indexofcheaper = aheadportsindex  # if found reset index of cheapest port
                            cheapestrate = airports[aheadportsindex].currencyeurorate  # grab cheapest rate of
                            break
                        else:
                            pass
                            # legidforcheapest = stopped_at_port + 1 + idx2
                    # print('in for x in range ..ateach to indexof cheaper')
                    if stopped_at_port == indexofcheaper:  ## then there is no cheaper airport within range
                        ...
                        for x in range(stopped_at_port,
                                       indexofcheaper + 1):  # want indexes of legs to reach cheaper port so add one to the index of the cheaper
                            distancetocheaper += legs[
                                x]  # distance to cheaper is the accumulated distance of all legs to reach it
            ...
            if routenumber < 24:  ##for routes with five legs
                ...
                for stopped_at_port in range(5):  # for each leg of the journey in this case five legs
                    ...
                    for idx2, aheadports in enumerate(subroute):  ## see if there is a cheaper port ahead within range
                        ...
                        aheadportsindex = stopped_at_port + 1 + idx2  ## compute the proper index of a port in subroute as subroute uses its own
                        # print('ahead airport index ',aheadportsindex,'aheadairport name',aheadports)
                        # print('aheadports indexs', aheadportsindex)
                        if (airports[
                                aheadportsindex].currencyeurorate <= cheapestrate):  ##find next cheapest airport as will buy enough fuel to get to it
...
def main():
    ...
    print('fuel to buy strategy in euro', z.fuelstrategy)
    print('legs distances', z.cheapestlegs)
    print('purchase strategy for each leg', z.purchase_strategy)
    print('cost of the route ', z.costofcheapest)
